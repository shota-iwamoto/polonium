...
>
> 仕様は [spec/safety-spec.md](../docs/spec/safety-spec.md)、設計は
> [design/ownership.md](../docs/design/ownership.md) / [design/error-handling.md](../docs/design/error-handling.md) /
> [design/os-support.md](../docs/design/os-support.md)。**この 4 つが実装の唯一の正解**です。
...
- 場所（Place）の表現と重なり判定（[ownership.md §3](../docs/design/ownership.md)）
...
- 一時値の drop（文の終わり）
- `del x`（早期解放）
- `runtime/` に `pl_drop_*` を追加（**core 側**。[os-support.md §4](../docs/design/os-support.md)）
- [design/memory-model.md](../docs/design/memory-model.md) を改訂（「解放しない」→「所有者が解放する」）
...
- 典型パターンごとの直し方は [ownership.md §8](../docs/design/ownership.md)
...
**目的**：握りつぶせないエラー処理（[error-handling.md](../docs/design/error-handling.md)）。
